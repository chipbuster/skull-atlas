var atom_color = {
    'H': [0.5, 0.5, 0.5], // ligth gray
    'C': [0.2, 0.2, 0.2], // dark gray (quite black)
    'N': [0., 0., 1.0], // blue
    'O': [0.8, 0.1, 0.], // red
    'F': [0.0, 0.75, 1.0], // ligth blue
    'P': [249./255, 157./255, 27./255], // orange
    'S': [254./255, 238./255, 3./255], // yellow,
    'Cl': [0., 1.0, 0.], //green
    'K': [200./255, 162./255, 200./255] // lilac
};
...
var atom_info = {
    ...
    "CA":["carbon",70,67,170,77,12.0107],
...
function Atom(options) {
    ...
    var seq = options.seq || -1,
        label = options.label || "",
        amino_label = options.amino_label || 'XXX',
        amino_seq = options.amino_seq || -1,
        position = options.position || [0., 0., 0.],
        symbol = label[0];

    this.get_name = function () {
        return atom_info[symbol][0];
    };
    ...
    this.get_mass = function () {
        return atom_info[symbol][5];
    };

    this.get_radius_vdw = function () {
        return atom_info[symbol][3];
    };

    this.get_color_0x = function () {
        var color = new THREE.Color();
        color.setRGB(atom_color[symbol][0], atom_color[symbol][1], atom_color[symbol][2]);

        return color.hex;
    };
};

function parse_PDB(name, text, callback) {
    var row_start,
        row_end,
        row,
        row_values,
    ...
    while (row_end != '-1') {
        ...
        if (row.indexOf('ATOM') == 0) {
            ...
            atom_list.push(new Atom({
                                        seq: Number(row_values[1]),
                                        label: row_values[2],
...
function create_PDB_nodes(atom_list, callback) {
    var nodes = {
        'H': [],
        'C': [],
        'N': [],
    ...
    atom_list.forEach(function (atom) {
        var pos = atom.get_position(),
            atom_node = {
                            type: "translate",
                            // id: "my-translate",
                            ...
                            nodes: [
                                    {
                                        type: "sphere",
                                        // id: "sphere1",
                                        radius: atom.get_radius_vdw()/100.,
                                        slices: 6,
                                        rings: 12
                                    }
                                   ]
                        };
        nodes[atom.get_symbol()].push(atom_node);
    });
